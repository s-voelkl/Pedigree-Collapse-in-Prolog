...
% coefficient is XXXX if the person does not have any parents, as there can be no pedigree collapse from that.
% number of parents == 0
coefficient(Coefficient, Person) :-
    % has no parents:
    not(any_parent_existing(Person)),
    
    % for testing: has not 2 parents:
    %not(both_parents_existing(Person)),
    Coefficient #= 0.

% number of parents == 1
coefficient(Coefficient, Person) :- 
    parent(Parent1, Person), not((parent(Parent2, Person), Parent1 \== Parent2)),
    generation_difference(GenDiffParent1, Person, Parent1),
    
    Coefficient is 100 + GenDiffParent1.
...
coefficient(Coefficient, Person) :- 
    ...
    % get the common ancestor of 2 parents
    parents_of_child(Parent1, Parent2, Person),
    find_common_ancestor(Parent1, Parent2, [], CommonAncestor),
    %common_ancestor(CommonAncestor, Parent1, Parent2),
    
    
    % get the generation differences between the parent and the common ancestor
    generation_difference(GenDiffParent1, Parent1, CommonAncestor),
    generation_difference(GenDiffParent2, Parent2, CommonAncestor),
    
    % debug:
    write('Person: '), write(Person), nl,
    write('Parent1: '), write(Parent1),
    write(',   GenDiffParent1: '), write(GenDiffParent1), nl,
    write('Parent2: '), write(Parent2),
    write(',   GenDiffParent2: '), write(GenDiffParent2), nl,nl,
    
    Coefficient is GenDiffParent1 + GenDiffParent2 +1. 
    
    %list_sum(ListOfCommonAncestorCoefficients, Coefficient).

	
find_common_ancestor(Parent1, Parent2, Visited, CommonAncestor) :-
    common_ancestor(CommonAncestor, Parent1, Parent2),
    \+ member(CommonAncestor, Visited),
    !.
find_common_ancestor(Parent1, Parent2, Visited, CommonAncestor) :-
    common_ancestor(Ancestor, Parent1, Parent2),
    \+ member(Ancestor, Visited),
    find_common_ancestor(Parent1, Parent2, [Ancestor|Visited], CommonAncestor).
