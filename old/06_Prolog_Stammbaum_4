...
coi(COI, Person) :- 
    ...
    % debug
    write(Person), nl, write(ListCommonAncestors), nl, write(CommonAncestorCOIs), nl, write(COI),
    
    % findall CommonAncestorCOIs
    common_ancestor_cois(CommonAncestorCOIs, ListCommonAncestors, Person),
    
    % COI is the sum of all CommonAncestorCOIs
    list_sum(COI, CommonAncestorCOIs).
    
    
%calc_coefficient(Coefficient, Person) :- 
  %  .



% base case: no CAsToVisit existing.
% TEST: common_ancestor_cois([],[], leo) -> true
common_ancestor_cois([], _, _):- write('base case 1').
common_ancestor_cois([], [], _) :- write('base case 2').

% base case: no CAsToVisit remaining anymore after this run.
common_ancestor_cois(CommonAncestorCOIs, [CurrentCA | Rest], Person) :-
    write('base case 3'),
    write('Processing common ancestor: '), write(CurrentCA), nl,
    
    write(CurrentCA), nl, write(Rest), nl, 
    parents_of_child(Parent1, Parent2, Person),
    generation_difference(GenDiffParent1, Parent1, CurrentCA),
    generation_difference(GenDiffParent2, Parent2, CurrentCA),
    write(GenDiffParent1), write(GenDiffParent2), write(CurrentCOI),
    
    CurrentCOI is (0.5 ** (GenDiffParent1 + GenDiffParent2 + 1)) * (1 + 0), % redo!
	append(CommonAncestorCOIs, [CurrentCOI], UpdatedCOIs),

    
common_ancestor_cois(CommonAncestorCOIs, [CurrentCA, CAsToVisit], Person) :-
    write('recursion'),
	% cut the current CommonAncestor (CA) from the CAsToVisit-List and use it for the calculation
    
    common_ancestor_cois([CurrentCOI | CommonAncestorCOIs], CAsToVisit, Person),
    ...
    CurrentCOI is (0.5 ** (GenDiffParent1 + GenDiffParent2 + 1)) * (1 + 0).
    %redo!
